...
public class CursorableLinkedList extends AbstractLinkedList implements Serializable {
    ...
    protected transient List<WeakReference<Cursor>> cursors = new ArrayList<WeakReference<Cursor>>();
    ...
    @Override protected void init() {
        super.init();
        cursors = new ArrayList<WeakReference<Cursor>>();
    }
     ...
     * @return a new cursor iterator
     */
    @Override public ListIterator listIterator(int fromIndex) {
     ...
     */
    @Override protected void updateNode(Node node, Object value) {
     ...
     */
    @Override protected void addNode(Node nodeToInsert, Node insertBeforeNode) {
     ...
     */
    @Override protected void removeNode(Node node) {
    ...
    protected void registerCursor(Cursor cursor) {
        ...
        for (Iterator<WeakReference<Cursor>> it = cursors.iterator(); it.hasNext();) {
            WeakReference ref = it.next();
            if (ref.get() == null) {
                it.remove();
            }
        }
        cursors.add(new WeakReference<Cursor>(cursor));
    }
    ...
    protected void broadcastNodeChanged(Node node) {
        Iterator<WeakReference<Cursor>> it = cursors.iterator();
        while (it.hasNext()) {
            WeakReference ref = it.next();
    ...
    public static class Cursor extends AbstractLinkedList.LinkedListIterator {
         ...
         */
        @Override protected void checkModCount() {
            if (!valid) {
