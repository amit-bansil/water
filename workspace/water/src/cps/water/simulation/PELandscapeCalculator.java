 ...
 * @author Amit Bansil
 */
class PELandscapeCalculator {
	private final int latRes;
	private final double CUTOFF;
	private final int[] pematrix;
	private final Engine raw;
	
	public PELandscapeCalculator(int latRes,double CUTOFF,Engine raw) {
	...
	public void generatePELandscape(ArrayFinal<Ion> ions,
		...
		double frang = 1.0d * this.raw.boxhx;
		...
		latx = this.raw.bx / (2 * latRes + 1);
		laty = this.raw.by / (2 * latRes + 1);
		latz = this.raw.bz / (2 * latRes + 1);
			...
			for (x = xmin; x <= xmax; x++) {
				...
				if (x == Math.round(((xPosition) * (latRes * 2)) - latRes)) {// """""""""""""""""
					...
					if (ions.getLength() == 1) xnli[0] = xnl;
						 ...
						 * won't work!!!
						 */
						xnli[0] = xnli[0] - 2.0 * this.raw.boxhx
								* ((int) (xnli[0] / this.raw.boxhx));
						xnli[1] = xnli[1] - 2.0 * this.raw.boxhx
								* ((int) (xnli[1] / this.raw.boxhx));
					}
						...
						if ((zmax <= latRes)) // System.out.println("???");
							for (z = zmin; z <= zmax; z += zstep) {
								...
								for (i = 0; i < ions.getLength(); i++) {
									...
									qqhyio = qion[i] * this.raw.q[1] * Engine.scfacq;
									qqoxio = qion[i] * this.raw.q[2] * Engine.scfacq;
									...
									for (j = 0; j < this.raw.mols; j++) {
										xnjo = this.raw.rn[j][0][2];
										ynjo = this.raw.rn[j][1][2];
										znjo = this.raw.rn[j][2][2];

										xx = (xnli[i] - xnjo) / this.raw.boxhx;
										yy = (ynli[i] - ynjo) / this.raw.boxhy;
										zz = (znli[i] - znjo) / this.raw.boxhz;

										iax = ((int) (xx)) + ((int) (xx));
										iay = ((int) (yy)) + ((int) (yy));
										iaz = ((int) (zz)) + ((int) (zz));

										abx = ((iax)) * (this.raw.boxhx);
										aby = ((iay)) * (this.raw.boxhy);
										abz = ((iaz)) * (this.raw.boxhz);
									...
									for (j = 0; j < this.raw.mols; j++) {
										xx = xion[j] - this.raw.rn[j][0][0];
										yy = yion[j] - this.raw.rn[j][1][0];
										zz = zion[j] - this.raw.rn[j][2][0];
									...
									for (j = 0; j < this.raw.mols; j++) {
										xx = xion[j] - this.raw.rn[j][0][1];
										yy = yion[j] - this.raw.rn[j][1][1];
										zz = zion[j] - this.raw.rn[j][2][1];
									...
									for (j = 0; j < this.raw.mols; j++) {
										xo = this.raw.rn[j][0][2];
										yo = this.raw.rn[j][1][2];
										zo = this.raw.rn[j][2][2];
