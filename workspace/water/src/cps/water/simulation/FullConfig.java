...
public final class FullConfig extends SimConfig {
    ...
    @Override public void fromData(final Engine data){
    ...
    @Override public void fromText(InputStream is) throws IOException{
	    ByteArrayOutputStream baos=new ByteArrayOutputStream();
	    int i=is.read();
	    while(i!=-1){
	  		baos.write(i);
    ...
    @Override public void toData(Engine data){
    	try{load(new DataInputStream(new ByteArrayInputStream(dat)),data);}
    	catch(IOException ex){ ex.printStackTrace(); }
    }
    
    public static final void load(final DataInputStream dis,final Engine data) throws IOException{
...
//System.err.println("S"+data.indxww[i][0]+","+data.indxww[i][1]+","+data.indxww[i][2]);
    ...
    public static final void save(final Engine data,final OutputStream os) throws IOException{
    ...
    private static final float convert(char v,double max,double min){
        return (float)( ( (((float)v)/((float)Character.MAX_VALUE))*(max-min))+min);
    }
}