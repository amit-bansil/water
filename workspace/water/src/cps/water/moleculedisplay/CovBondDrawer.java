...
public class CovBondDrawer {
	...
	public void draw(GLAccess gla,float[] positions,int count, float[] boxSize) {
		...
		//need this since no display list don't handle normals correctly
		gla.setColor(Renderer.hColor);
		GLAccess.Quadrics quadrics = gla.createQuadrics();
	...
	private void _draw(GLAccess gla,Quadrics quadrics) {
		  ...
		  //can't use display lists since they don't scale normals correctly 
		  //even with GL_NORMALIZE, see VMD-OpenGLRenderer.c for more info
		  //gla.scale(1,1, (float)R);
		  //displayList.call();
		  quadrics.cylinder(.02f, (float)R, Renderer.covBondSlices);
