...
public class Application {
		...
		Link simChangeL=new Link() {
			@Override protected void signal() {
				if(model.primarySelection().get()!=primary||model.secondarySelection().get()!=secondary) {
					...
					if(primary!=null) primary.getDisplay().getSource().connect(displayChangeL);
					if(secondary!=null) secondary.getDisplay().getSource().connect(displayChangeL);
	...
	private boolean renderingUpToDate=false;
	...
	private boolean scenesRead=true;
