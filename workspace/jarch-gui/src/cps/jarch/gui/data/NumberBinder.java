		...
		final ChangeListener targetChangeL=new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				...
				if (newV != curV) {
					...
					//also clamp it to avoid rounding out of bounds
					double actMin=((Number) m.getMinimum()).doubleValue();
					double actMax=((Number) m.getMaximum()).doubleValue();
					double actV=(target.getValue() * minStep)+actMin;
					if(actV>actMax)actV=actMax;
					if(actV<actMin)actV=actMin;
					source.setValue(actV);
				}
			}
		};
