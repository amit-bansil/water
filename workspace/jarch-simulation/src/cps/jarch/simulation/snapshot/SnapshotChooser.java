...
public class SnapshotChooser<SnapType extends Snapshot> {
	...
	private final class SnapshotComponent implements Unlinker{
		...
		private final JComponent thumbnail= new JComponent(){
			...
			private final void drawOverlay(Graphics2D g,int x,int y,int w,int h) {
				...
				if(model.focusedSelection().get()==snap) {
					overlayColor=FocusedOverlayColor;
	...
	private static final Color FocusedOverlayColor=Color.orange.darker().darker().darker();
	...
	private static final Border TitleBorder=BorderFactory.createEmptyBorder(MED,0,MED,0);
	private static final Border ButtonsBorder=BorderFactory.createEmptyBorder(MED,0,0,0);
	private static final Border FocusedThumbnailBorder=BorderFactory.createLineBorder(FocusedColor, MED);
	private static final Border NormalThumbnailBorder=BorderFactory.createLineBorder(Color.BLACK, MED);
	private static final Border UnfocusedThumbnailBorder=BorderFactory.createLineBorder(UnfocusedColor,MED);
	private static final Composite OverlayComposite=AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.9f);
	...
	private final class SnapshotComponents extends ListView<SnapType,SnapshotComponent>{
		...
		@Override protected SnapshotComponent create(SnapType newIn, int index) {
			...
			titles.add(c.titleHolder,index);
	...
	public SnapshotChooser(final SnapshotsModel<SnapType> model) {
		...
		JComponent innerContent=new JPanel(layout);
		...
		titles = new JPanel(new GridLayout(1, 0,12,12));

		thumbnails = new JPanel(new GridLayout(1, 0,12,12));
		...
		innerContent.add(thumbnails, Position.Center);
		innerContent.add(titles, Position.South);
