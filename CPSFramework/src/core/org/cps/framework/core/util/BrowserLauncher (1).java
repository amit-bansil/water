...
public class BrowserLauncher {
	...
	/**
		...
		*/
	//AB CHANGED TO PROTECTED TO STOP WARNINGS
	//CAN'T HURT??
	//TEST MACOS X
	...
	public static void openURL(String url) throws IOException {
		...
		Object browser_l = locateBrowser();
		if (browser_l == null) {
		...
		switch (jvm) {
			case MRJ_2_0 :
				...
				try {
					...
					sendNoReply.invoke(browser_l, new Object[] {
					});
			...
			case MRJ_2_1 :
				Runtime.getRuntime().exec(
					new String[] {(String) browser_l, url });
				break;
			case MRJ_3_0 :
		...
		switch (jvm) {
			case MRJ_2_0 :
				try {
					...
					sendNoReply =
						appleEventClass
							.getDeclaredMethod("sendNoReply", new Class[] {
					});
				...
				for (int i = 0; i < systemFolderFiles.length; i++) {
					try {
						...
						if (FINDER_TYPE.equals(fileType.toString())) {
							Object fileCreator =
								getFileCreator.invoke(
									null,
									new Object[] { file });
							if (FINDER_CREATOR
								.equals(fileCreator.toString())) {
						...
						//meaningless assignment removed by AB
						//browser = browser;
						errorMessage = iare.getMessage();
						return null;
					} catch (IllegalAccessException iae) {
