 ...
 *
 */
abstract class THashIterator implements Iterator {
    ...
    public THashIterator(TObjectHash hash) {
        _hash = hash;
        _expectedSize = hash.size();
        _index = hash.capacity();
    }
    ...
    /**
     * Returns true if a call to next() would return a value instead
    ...
    /**
     * Removes the last entry returned by <tt>next()</tt>.
     * Invoking this method more than once for a single next()
